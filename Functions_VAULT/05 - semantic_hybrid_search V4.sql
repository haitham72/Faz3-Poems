...
BEGIN
  ...
  WITH fts_search AS (
    SELECT 
      ...
      ds.metadata,
      GREATEST(
        ts_rank_cd(ds.fts, websearch_to_tsquery('arabic', query_text)) * 2.0,
        ts_rank_cd(to_tsvector('arabic', COALESCE(ds.metadata->>'summary', '')), websearch_to_tsquery('arabic', query_text)) * 1.2
      )::float as rank
    FROM document_semantic ds
    WHERE 
      ds.fts @@ websearch_to_tsquery('arabic', query_text) OR
      to_tsvector('arabic', COALESCE(ds.metadata->>'summary', '')) @@ websearch_to_tsquery('arabic', query_text)
  ),
  semantic_search AS (
    SELECT 
      ds.id,
      ...
      ds.metadata->>'summary' as summary,
      ds.metadata,
      (1.0 - (ds.embedding <=> query_embedding))::float as similarity
    FROM document_semantic ds
  ...
  fuzzy_search AS (
    SELECT 
      ...
      ds.metadata,
      GREATEST(
        similarity(ds.content, query_text) * 2.0,
        similarity(COALESCE(ds.metadata->>'summary', ''), query_text) * 1.5,
        similarity(COALESCE(ds.metadata->>'Title_raw', ''), query_text) * 1.0
      )::float as fuzzy_sim
    FROM document_semantic ds
    WHERE 
      ds.content % query_text OR
      (ds.metadata->>'summary') IS NOT NULL AND (ds.metadata->>'summary')::text % query_text OR
      (ds.metadata->>'Title_raw') IS NOT NULL AND (ds.metadata->>'Title_raw')::text % query_text
  ),
  combined AS (
    SELECT 
      ...
      COALESCE(fz.fuzzy_sim, 0.0) as fuzzy_score
    FROM fts_search f
  ...
  SELECT 
    ...
    c.semantic_score,
    c.fuzzy_score,
    (COALESCE(c.fts_score, 0.0) * fts_weight + 
     COALESCE(c.semantic_score, 0.0) * semantic_weight + 
     COALESCE(c.fuzzy_score, 0.0) * fuzzy_weight) as final_score
  FROM combined c
  WHERE 
    (COALESCE(c.fts_score, 0.0) * fts_weight + 
     COALESCE(c.semantic_score, 0.0) * semantic_weight + 
     COALESCE(c.fuzzy_score, 0.0) * fuzzy_weight) >= 0.5
