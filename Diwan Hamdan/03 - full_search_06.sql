...
BEGIN
    ...
    final_results AS (
        ...
        SELECT 
            ...
            fm.poem_id as p_id,
            MIN(fm."Row_ID") as r_id,
            MAX(fm."Title_cleaned") as t_clean,
            NULL::TEXT as pl_clean,
            MAX(fm.matched_cols) as m_cols,
            (array_agg(fm.highlight_data))[1] as h_data,
            2 as prio
        FROM filtered_matches fm
        WHERE fm.title_matched = TRUE
        AND fm.poem_id NOT IN (
            SELECT fm2.poem_id FROM filtered_matches fm2 WHERE fm2.line_matched = TRUE
        )
        GROUP BY fm.poem_id
        ...
        SELECT 
            ...
            fm.poem_id as p_id,
            fm."Row_ID" as r_id,
            fm."Title_cleaned" as t_clean,
            fm."Poem_line_cleaned" as pl_clean,
            fm.matched_cols as m_cols,
            fm.highlight_data as h_data,
            fm.calc_priority as prio
        FROM filtered_matches fm
        WHERE fm.line_matched = TRUE
           OR (fm.line_matched = FALSE AND fm.title_matched = FALSE AND array_length(fm.matched_cols, 1) > 0)
    )
