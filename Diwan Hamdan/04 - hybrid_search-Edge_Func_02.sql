  ...
  try {
    ...
    let finalResults = searchResults.map((result: any) => {
      ...
      return {
        ...
        title: meta.Title_raw || meta.title_cleaned,
        ...
        // Only include entities/subjects if matched
        ...(matchedMetadata.length > 0 && {
          matched_entities: meta.entities?.map((e: any) => e.name).slice(0, 3),
          matched_subjects: meta.subjects?.slice(0, 3)
        })
      }
    })
