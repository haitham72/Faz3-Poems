...
serve(async (req) => {
  ...
  try {
    ...
    // Fallback: use context_query if full_context_query not provided
    const semanticQuery = full_context_query || context_query;

    console.log("🔍 Query (original):", query);
    console.log("🏷️ Context query (entities):", context_query);
    console.log("🎯 Semantic query (intent):", semanticQuery);
    ...
    // =====================================================
    // OPTIMIZED WEIGHTS (Content 42%, Summary 23%, Metadata 21%, Exact 14%)
    // =====================================================
    const DEFAULT_WEIGHTS = {
      // Content (42%) - primary source of truth
      content_fts_weight: 0.12, // Exact phrase matching
      content_trigram_weight: 0.08, // Fuzzy matching
      content_semantic_weight: 0.22, // Main semantic signal

      // Summary (23%) - thematic understanding
      summary_fts_weight: 0.05, // Keywords in summary
      summary_semantic_weight: 0.18, // Thematic matching (BOOSTED from 0.15)

      // Metadata (21%) - context validation
      entity_weight: 0.08,
      subject_weight: 0.06,
      place_weight: 0.04,
      event_weight: 0.02,
      religion_weight: 0.01,

      // Exact (14%) - precision boost
      exact_weight: 0.14,
    };

    const finalWeights = { ...DEFAULT_WEIGHTS, ...weights };

    console.log("⚖️ Optimized Weights:", finalWeights);

    // =====================================================
    // TWO DIFFERENT EMBEDDINGS FOR DIFFERENT PURPOSES
    // =====================================================
    const embeddingPromises = [
      // Content embedding - USE CONTEXT_QUERY (full entities for detailed matching)
      fetch("https://api.openai.com/v1/embeddings", {
        ...
        body: JSON.stringify({
          ...
          input: context_query.trim(),
          encoding_format: "float",
        }),
      }),
      // Summary embedding - USE SEMANTIC_QUERY (clean intent/theme only)
      fetch("https://api.openai.com/v1/embeddings", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${openaiKey}`,
    ...
    console.log("   - Content: from context_query (entities)");
    console.log("   - Summary: from semantic_query (intent)");
    ...
    const { data: searchResults, error: searchError } = await supabase.rpc(
      "hybrid_search_opus",
      {
        query_text: query, // Original for FTS/trigram
        context_query_text: context_query, // For exact match detection
        query_embedding: queryEmbedding, // Context query embedding (entities)
        summary_embedding: summaryEmbedding, // Semantic query embedding (intent)
    ...
    // Calculate absolute scores (×100 for readability)
