...
BEGIN
    ...
    trigram_results AS (
        SELECT
            d.id,
            ROW_NUMBER() OVER(ORDER BY similarity(normalize_arabic(d.content), q_norm) DESC) AS rank_ix,
            similarity(normalize_arabic(d.content), q_norm)::FLOAT AS score
        FROM documents d
        WHERE similarity(normalize_arabic(d.content), q_norm) > 0.1
        ORDER BY rank_ix
        LIMIT LEAST(match_count, 30) * 3
    ),
    ...
    exact_results AS (
        SELECT
            ...
            ROW_NUMBER() OVER(ORDER BY 
                CASE 
                    -- Title exact phrase = highest priority
                    WHEN normalize_arabic(d.metadata->>'title_cleaned') LIKE '%' || q_norm || '%' THEN 1
                    -- Content exact phrase
                    WHEN normalize_arabic(d.content) LIKE '%' || q_norm || '%' THEN 2
                    ...
                    -- Content contains all meaningful words
                    WHEN (
                        SELECT COUNT(*) = array_length(meaningful_words, 1)
                        FROM unnest(meaningful_words) mw
                        WHERE position(normalize_arabic(mw) IN normalize_arabic(d.content)) > 0
                    ) THEN 4
                    ELSE 5
                END
            ) AS rank_ix,
            CASE 
                ...
                WHEN normalize_arabic(d.content) LIKE '%' || q_norm || '%' THEN TRUE
                ELSE FALSE
            END AS has_exact_match
        FROM documents d
        WHERE 
            normalize_arabic(d.metadata->>'title_cleaned') LIKE '%' || q_norm || '%'
        ...
        CROSS JOIN LATERAL (
            SELECT unnest(ARRAY[
                ...
                CASE WHEN EXISTS (
                    ...
                    WHERE position(q_norm IN normalize_arabic(entity->>'name')) > 0
                       OR EXISTS (
                       ...
                       OR EXISTS (
                           SELECT 1 FROM unnest(meaningful_words) mw
                           WHERE position(normalize_arabic(mw) IN normalize_arabic(entity->>'name')) > 0
                              OR EXISTS (
                                  SELECT 1 FROM jsonb_array_elements_text(entity->'resolved_from') rf
    ...
    SELECT
        ...
        ((c.fts_rrf + c.trigram_rrf + c.exact_rrf + c.semantic_rrf) * 100 + 
         array_length(c.metadata_fields, 1) * 60)::FLOAT AS final_score,
