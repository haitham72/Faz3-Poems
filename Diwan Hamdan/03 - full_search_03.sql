        ...
        SELECT 
            ...
            jsonb_build_object(
                'matched_columns', jsonb_build_array(
                    ...
                    CASE WHEN search_animals 
                         AND d.animals IS NOT NULL 
                         AND jsonb_typeof(d.animals) = 'array'
                         AND (
                        -- Match ANY text in animals JSONB
                        d.animals::text ILIKE '%' || q || '%'
                    ) THEN 'animals' ELSE NULL END,
                ...
                -- Animals (ONLY if animals search is enabled) - Same as entities
                'animals', CASE WHEN search_animals THEN (
                    SELECT jsonb_agg(
                        jsonb_build_object(
                            'semantic_name', a->>'name',
                            'type', a->>'type',
                            'matched_alias', (
                                SELECT alias
                            ...
                            'matched_semantic', CASE 
                                WHEN a->>'name' ILIKE '%' || q || '%' THEN true
                                ELSE false
                            END,
                            'all_aliases', a->'resolved_from'
                        )
                    )
