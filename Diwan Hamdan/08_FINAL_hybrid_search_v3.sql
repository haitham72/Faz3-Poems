...
BEGIN
    ...
    content_semantic_results AS (
        SELECT
            d.id,
            ROW_NUMBER() OVER(ORDER BY d.embedding <#> hybrid_search_opus.query_embedding) AS rank_ix,
            (1 - (d.embedding <=> hybrid_search_opus.query_embedding))::FLOAT AS score
        FROM documents d
    ...
    summary_semantic_results AS (
        SELECT
            d.id,
            ROW_NUMBER() OVER(ORDER BY d.summary_embedding <#> hybrid_search_opus.summary_embedding) AS rank_ix,
            (1 - (d.summary_embedding <=> hybrid_search_opus.summary_embedding))::FLOAT AS score
        FROM documents d
