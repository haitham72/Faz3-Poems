...
function App() {
  ...
  const N8N_WEBHOOKS = {
    aiSummary: "http://localhost:5678/webhook/ai-summary",
    exactSearch: "http://localhost:5678/webhook/exact-search",
    semanticSearch: "http://localhost:5678/webhook/semantic-search",
  };
  ...
  const performFullSearch = async (searchQuery) => {
    ...
    fetch(N8N_WEBHOOKS.aiSummary, {
      ...
      .then((aiResponse) => {
        ...
        if (aiResponse) {
          // Format 1: OpenAI format
          if (aiResponse.choices?.[0]?.message?.content) {
            console.log("✓ Found AI Summary (OpenAI format)");
            setAiSummary(aiResponse.choices[0].message.content);
          }
          // Format 2: Array with summary key - [{"summary": "..."}]
          else if (Array.isArray(aiResponse) && aiResponse[0]?.summary) {
            console.log("✓ Found AI Summary (summary key)");
            setAiSummary(aiResponse[0].summary);
          }
          // Format 3: Array with message
          else if (
          ...
          // Format 4: Simple {response: "text"} format (for testing)
          else if (aiResponse.response) {
            console.log("✓ Found AI Summary (simple format)");
            setAiSummary(aiResponse.response);
          } else {
            console.warn("⚠️ AI Summary format not recognized:", aiResponse);
          }
        }
        checkCompletion();
      })
      ...
      .then((exactResponse) => {
        ...
        if (exactResponse) {
          ...
          if (exactResponse.results && Array.isArray(exactResponse.results)) {
            console.log(
              `✓ Found ${exactResponse.results.length} Exact Results (results key)`
            );
            setExactResults(exactResponse.results);
          }
          // Format 2: [{"hybrid_search_exact": [...]}]
          else if (
            Array.isArray(exactResponse) &&
            exactResponse[0]?.hybrid_search_exact
          ) {
            console.log(
              `✓ Found ${exactResponse[0].hybrid_search_exact.length} Exact Results (hybrid_search_exact key)`
            );
            setExactResults(exactResponse[0].hybrid_search_exact);
          }
          // Format 3: Direct array
          else if (Array.isArray(exactResponse)) {
            console.log(
              `✓ Found ${exactResponse.length} Exact Results (direct array)`
            );
            setExactResults(exactResponse);
          }
          // Format 4: Simple {response: "text"} format (for testing)
          else if (exactResponse.response) {
            console.log("✓ Found Exact Results (simple format)");
            setExactResults([
              {
                id: Date.now(),
                poem_name: "نتيجة تجريبية",
                content: exactResponse.response,
                scores: { final: 1.0 },
              },
            ]);
          } else {
            console.warn(
              "⚠️ Exact Response format not recognized:",
              exactResponse
            );
          }
        }
      ...
      .then((semanticResponse) => {
        ...
        if (semanticResponse) {
          ...
          if (
            ...
            Array.isArray(semanticResponse.results)
          ) {
            console.log(
              `✓ Found ${semanticResponse.results.length} Semantic Results (results key)`
            );
            setSemanticResults(semanticResponse.results);
          }
          // Format 2: [{"hybrid_search_semantic": [...]}]
          else if (
            Array.isArray(semanticResponse) &&
            semanticResponse[0]?.hybrid_search_semantic
          ) {
            console.log(
              `✓ Found ${semanticResponse[0].hybrid_search_semantic.length} Semantic Results (hybrid_search_semantic key)`
            );
            setSemanticResults(semanticResponse[0].hybrid_search_semantic);
          }
          // Format 3: {semantic: [...]}
          else if (
            semanticResponse.semantic &&
            Array.isArray(semanticResponse.semantic)
          ) {
            console.log(
              `✓ Found ${semanticResponse.semantic.length} Semantic Results (semantic key)`
            );
          ...
          // Format 4: [{ semantic: [...] }]
            ...
            Array.isArray(semanticResponse) &&
            semanticResponse[0]?.semantic
          ) {
            console.log(
              `✓ Found ${semanticResponse[0].semantic.length} Semantic Results (semantic array format)`
            );
          ...
          // Format 5: Direct array
            ...
            console.log(
              `✓ Found ${semanticResponse.length} Semantic Results (direct array)`
            );
            setSemanticResults(semanticResponse);
          }
          // Format 6: Simple {response: "text"} format (for testing)
          else if (semanticResponse.response) {
            console.log("✓ Found Semantic Results (simple format)");
            setSemanticResults([
              {
                id: Date.now() + 1,
                poem_name: "نتيجة تجريبية - دلالية",
                content: semanticResponse.response,
                scores: { final: 1.0 },
              },
            ]);
          } else {
            console.warn(
              "⚠️ Semantic Response format not recognized:",
              semanticResponse
            );
          }
        }
  ...
  return (
    <div className="app">
      <div className="container">
        ...
        {/* Results Grid - Shows skeletons OR real content */}
        {(loading ||
          stats ||
          aiSummary ||
          ...
          <div className="results-grid">
            ...
            {/* AI Summary - Show skeleton OR real content */}
            {loading && !aiSummary ? (
            ...
            {/* Exact Search - Show skeleton OR real content */}
            {loading && exactResults.length === 0 ? (
              <div className="card poems-card full-width skeleton-card">
                <div className="card-header">
                  <div className="skeleton skeleton-icon"></div>
                  <div className="skeleton skeleton-title"></div>
                </div>
                <div className="skeleton skeleton-poem"></div>
                <div className="skeleton skeleton-poem"></div>
              </div>
            ) : uniqueExactResults.length > 0 ? (
              <div className="card poems-card full-width">
                ...
                <div className="poems-list">
                  {uniqueExactResults.map((poem, index) => (
                    <div
                      key={`exact-${poem.id}-${index}`}
                      className="poem-item exact-match"
                    >
                      ...
                      {poem.scores && (
                        <div className="score-breakdown">
                          <span>
                            دلالي: {(poem.scores.vector * 100).toFixed(0)}%
                          </span>
                          <span>
                            كلمات: {(poem.scores.keyword * 100).toFixed(0)}%
                          </span>
                          <span>
                            نمط: {(poem.scores.pattern * 100).toFixed(0)}%
                          </span>
                          <span>
                            تشابه: {(poem.scores.trigram * 100).toFixed(0)}%
                          </span>
                        </div>
                      )}
                    </div>
            ...
            {/* Semantic/Thematic Results - Show skeleton OR real content */}
            {loading && semanticResults.length === 0 ? (
              ...
              <div className="card poems-card full-width">
                <div className="card-header">
                  ...
                  <h3>
                    نتائج المعنى والموضوع ({uniqueSemanticResults.length})
                  </h3>
                </div>
                <div className="poems-list">
                  {uniqueSemanticResults.map((poem, index) => (
                    <div
                      key={`semantic-${poem.id}-${index}`}
                      className="poem-item semantic-match"
                    >
                      <div className="poem-header">
                        <div>
                          <h4 className="poem-title">{poem.poem_name}</h4>
                          <span className="match-badge semantic">
                            مطابقة معنوية
                          </span>
                        </div>
                        {poem.scores && (
                      ...
                      {poem.scores && (
                        <div className="score-breakdown">
                          ...
                          <span>
                            كيانات: {poem.scores.entity?.toFixed(1) || 0}
                          </span>
                          <span>
                            مشاعر: {poem.scores.sentiment?.toFixed(1) || 0}
                          </span>
                          <span>
                            تشابه: {(poem.scores.trigram * 100).toFixed(0)}%
                          </span>
